...
from insertion.pattern.tree import Tree, Text, Sentence, Word, table, WORD, POS, CHUNK, PNP

DEFAULT = "default"

from insertion.pattern.helpers import encode_string, decode_string
...
def train_topmine_ngrammer(documents, threshhold=1, max_ngramm_len=3, min_word_len=2, regexp="[.,!?;: ]",
                           stopwords=None):
    """
    ...
    for doc in documents:
        if isinstance(doc, str):
            splitted_docs.append(
                split_document_by_delimeters(doc, regexp, min_word_len=min_word_len, stopwords=stopwords))
...
def pprint(string, token=None, column=4):
...
class Morphology(lazylist):
    ...
    def __init__(self, path="", known=None):
        ...
        self.known = known
        self._path = path
        self._cmd = {"word", "char", "haspref", "hassuf", "addpref", "addsuf", "deletepref", "deletesuf", "goodleft",
                     "goodright"}
...
class Context(lazylist):
    ...
    def __init__(self, path=""):
        ...
        """
        self._path = path
        self._cmd = {"prevtag", "nexttag", "prev2tag", "next2tag", "prev1or2tag", "next1or2tag", "prev1or2or3tag",
                     "next1or2or3tag", "surroundtag", "curwd", "prevwd", "nextwd", "prev1or2wd", "next1or2wd",
                     "next1or2or3wd", "prev1or2or3wd", "prevwdtag", "nextwdtag", "wdprevtag", "wdnexttag", "wdand2aft",
                     "wdand2tagbfr", "wdand2tagaft", "lbigram", "rbigram", "prevbigram", "nextbigram"}
...
ABBREVIATIONS = \
    abbreviations = {"a.", "adj.", "adv.", "al.", "a.m.", "art.", "c.", "capt.", "cert.", "cf.", "col.", "Col.",
...
def find_tokens(string, punctuation=PUNCTUATION, abbreviations=ABBREVIATIONS, replace=replacements,
                linebreak=r"\n{2,}"):
